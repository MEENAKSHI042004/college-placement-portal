...
class UpdatePlacementStatusView(AdminRequiredMixin, View):
    def post(self, request, pk):
        ...
        if status in ["placed", "not_placed"]:
    ...
    def post(self, request):
        ...
        for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            try:
                ...
                if not all([full_name, email, password, roll_number, branch, year, cgpa]):
        ...
        if success_count:
            messages.success(request, f"{success_count} student(s) uploaded successfully.")

        if skipped_count:
            messages.warning(request, f"{skipped_count} student(s) skipped (already exist).")
...
class DownloadSampleTemplateView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        ...
        ws['A8']      = (
            'Valid Branches: CSE, ECE, MECH, CIVIL, EEE, IT, AIDS, AIML'
        )
